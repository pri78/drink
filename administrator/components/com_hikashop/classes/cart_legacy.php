...
class hikashopCartClass extends hikashopClass {
	...
	function loadCart($cart_id = 0, $reset = false) {
		...
		if(!empty($this->cart_id)){
			...
			if(!empty($hkUser)) {
				$this->filters[] = '(a.user_id = '.(int)$hkUser->user_id.')';
			}
	...
	function save(&$cart) {
		...
		} else if(!isset($cart->user_id) || !$app->isAdmin() || ($currUser->user_id == $cart->user_id || $currUser->user_cms_id == $cart->user_id) || (isset($cart->session_id) && $session->getId() == $cart->session_id)) {

			if($currUser->user_cms_id == $cart->user_id)
				$cart->user_id = (int)$currUser->user_id;
	...
	function initCart() {
		...
		if(!$app->isAdmin()) {
			...
			if(!empty($hkUser->user_id))
				$cart->user_id = (int)$hkUser->user_id;
			if($session->getId())
	...
	function updateEntry($quantity, &$cartContent, $product_id, $add, $resetCartWhenUpdate = true, $type = 'product', $force = false) {
		...
		if($quantity){
			if(!empty($cartContent) && isset($cartContent[$id])) { // && in_array($id,array_keys($cartContent))){
				if($add){
	...
	function _checkQuantity(&$product,&$quantity,&$cartContent,$cart_product_id_for_product) {
		...
		if(hikashop_level(1)) {
			...
			if( $limiters > 0 ) {
				...
				$user = JFactory::getUser();
				$hkUser = hikashop_loadUser(true);
				if(!empty($hkUser->user_id) && $baseDates[$dateLimiter] >= 0) {
	...
	function delete(&$id, $cart_date='new'){
		...
		$user = JFactory::getUser();
		$session = JFactory::getSession();
		if($app->isAdmin() || (!empty($currUser) && ($currUser->user_id == $cartInfo->user_id || $currUser->user_cms_id == $cartInfo->user_id)) || $session->getId() == $cartInfo->session_id) {
	...
	function convert($cart_id, $cart_type){
		...
		$session = JFactory::getSession();
		if($cart_id == 0 || $app->isAdmin() || $hkUser->user_id == $cartInfo->user_id || $hkUser->user_cms_id == $cartInfo->user_id || $session->getId()==$cartInfo->session_id){
			if($cart_type == 'cart'){
				$cart_type = 'wishlist';
				...
				if($cart_id == 0){
					...
					$newCart->user_id = (int)$hkUser->user_id;
	...
	function setCurrent($cart_id = '0', $cart_type = 'cart'){
		...
		if($cart_id == 0 || $app->isAdmin() || $hkUser->user_id == $cartInfo->user_id || $hkUser->user_cms_id == $cartInfo->user_id || $session->getId()==$cartInfo->session_id){
			...
			$query='UPDATE '.hikashop_table('cart').' SET cart_current = 0 WHERE (user_id='.(int)$hkUser->user_id.' OR session_id='.$db->quote($sessionToken).') AND cart_type='.$db->quote($cart_type).' AND cart_id != '.(int)$cart_id;
			$db->setQuery($query);
			$result = $db->query();
		}
		return $result;
